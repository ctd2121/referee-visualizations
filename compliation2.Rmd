...
subsetTeams %>%
  ...
  ggplot(., aes(x = factor(PenaltyTeam), y = PenPerGame)) +
    ...
    geom_text(aes(label = outlier), na.rm = TRUE, hjust = 1.1, size=2)+xlab('Team')+ylab('Average Penalties Per Game by Referee')+ggtitle('Avg Number of Penalties by Referee by Team: Outliers')+theme_ref
...
When analyzing Figure 8, remember that we are now looking at the difference between home and away penalties for only Brad Allen's referee crew. We can see that the range of difference can be quite large. The expected value of the distribution above is just above zero, but some teams in particular (`TEN`,`OAK`,`DET`,`LAC`) have large deviations from zero. We can see that on average, Brad Allen's crew calls more penalties on away teams than home teams, but there are several outliers where they call many more penalties against the home team than the away team. This type of analysis is useful to dig into more detail about the habits of each referee crew, and serves as further analysis for whether being the away team is a disadvantage when it comes to penalties. With future analysis, we hope to learn more about how each individual crew treats the away team when compared to the home team.

###Section 4G: Analysis by Yard Line

Next we examine the distribution of penalties that are called by location on the field. Figure 9 below shows this distribution, where a value of zero means the penalty was called as close as possible to the offensive team's own endzone, and a value of 100 means the penalty was called at the goal line of the attacking endzone.
...
Figure 10 shows the distribution of average yard lines at which penalties are called by each referee crew. The graph shows that the average yard line of penalties for all referees is between 46 and 49. This finding is consistent with the results from the histogram of `r figr('yardlines_1', prefix=TRUE)`, where we see that penalties are more frequently called at yard lines closer to the offensive team's endzone than to the defensive team's endzone.
