...
## Analysis by Quarter
```{r fig.cap="Figure 5: Penalties per Game by Quarter", fig.align= 'center'}
df2 <- refData
...
penalties_by_q <- ggplot(df_by_q, aes(x = Quarter, y = adj_count, width = 0.8)) +
...
```{r fig.cap="Figure 6: Penalties by Yard Line", fig.align= 'center'}
df <- refData
...
```{r fig.cap="Figure 7: Penalty Yard Line Averages by Referee", fig.align= 'center'}
library(tidyverse)
df_subset <- df %>%
  group_by(Referee) %>%
...
Once we combined the various data sources into a single dataset, we began exploring the data from multiple angles. An examination of the distribution of penalties by quarter prouced the graph below.
...
After some other high-level exploration of penalty distributions, we began comparing penalties called by referee. One of the most important findings of our analysis is conveyed in the following graph:

```{r fig.align = 'center', fig.height= 7, fig.width = 7.75}
HA_byRef
```
