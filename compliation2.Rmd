...
```{r ref_by_team, anchor='figure', fig.cap="Figure 6: Penalties by Referee by Team", fig.align= 'center'}
...
```{r ref_tendencies, anchor='figure', fig.cap="Figure 7: Penalties by Referee by Team: Outliers", fig.align= 'center'}
...
Remember that this boxplot is the same as in `r figr('ref_by_team', prefix=TRUE)`, only with the axis switched and data subsetted to only include those teams with outliers. We have labelled exactly which referee crews are the outliers for each team. One potential explanation for these deviances could be that particular referee crews are more likely to call specific penalties (for example, pass interference), and some teams are more likely to commit certain penalties. In this case there then there could be outliers created.  Another explanation is that there could be a limited number of data points for particular referee/team combos. This will be remedied as there is additional data added over time There are a few crews that appear on the chart multiple times. For example  John Hussey's crew appears three times on the chart, appearing to call more penalties on the New York Giants (NYG) and Atlanta (ATL), while less penalties on New England (NE). This could mean that the crew is slightly less consistent in their penalty calling.

###Section 4 F: Analysis by Referee by Team - Home vs Away


We were also interested in doing an extention of looking at referee by team, and decided to look at whether certain referees had a large difference in the number of penalties that they called on a team at away vs. the number of penalties that they called at home. We used a similar methodology to the previous section. First we determined the average number of penalties called when the team was away, and the average number of penalties called when the team was home. We then took the difference between means (Average # Away - Average # Home). This gave us a distribution of the differences bewteen home penalties and away penalties for each referee and each team. Because there is a huge number of categorical varaibles, we decided to focus on only the referee that has the highest variance between home and away: Brad Allen. You can see the average difference between away and home penalties by team for Brad Allen below.
```{r ref_home_away_team, anchor='figure', fig.cap="Figure 8: Penalties by Referee by Team: Home and Away", fig.align= 'center'}
...
ggplot(BradData,aes(x=reorder(Team,Difference),y=Difference,color=Outlier))+geom_point(size=2.5)+coord_flip()+theme_light()+ggtitle('Brad Allen: Difference between Away and Home for Each Team')+ylab('Average Penalties Away minus Average Penalties Home')+xlab('Team')+geom_hline(yintercept = mean(BradData$Difference),color = 'red', alpha = 0.3)+theme(axis.title.x = element_text(size = 16),
       axis.title.y = element_text(size = 16),
       plot.title = element_text(hjust = 0.5, size = 15))

```
Note that in order to determine outliers, we created a distriubtion for each team, based on the difference between away and home penalties. If the referee was an outlier for that particular team, that is represented in the above chart. So the outliers are not representative of being outliers for the particular referee, but instead outliers the team.

The outliers for Brad Allen include Tennessee (TEN), Los Angeles Charges (LAC), and Detroit (DET). Remember that the chart can be interpreseted as we move to the right, the referee calls more away penalties than he does home penalties. We can see that on average, Brad Allen's crew calls more penalties on away teams than home teams, but there are several outliers where they call much more penalties against the home team than the away team. This kind of analysis is useful to dig into more detail about the habits of each referee, and further analysis for whether being the away team is a disadvantage in terms of penalties. 

###Section 4 G: Analysis by Yardline
...
```{r anchor='figure', fig.cap="Figure 9: Penalties by Yard Line", fig.align= 'center'}
df <- refData
...
This component is to act as an interactive application that users can use to learn about how penalities within the NFL occur at different frequencies depending on field position.
...
The entire Interactive Component was coded using Native JavaScript and D3.js v4. This component can also be viewed as a standalone page by going to bit.ly/edavnfl
...
One challenge that we faced was vizualizing data for so many categorical variables. This is the case in the 'Analysis by Referee by Team' analysis above. Having 32 teams and 16 referee crews, means that visualizing anything beyond a boxplot becomes very difficult (although one thing that we learned through completing our project is just how useful the boxplot can be). One future path that we would like to take would be to create additional dynamic visualizations for our data. Allowing users to slice the data on different variables would allow us to show more, without overloading the report with charts. Alternatively, instead of looking at all teams and referees, in the future we could narrow our analysis to look at specific teams or specific referee crews. This would allow us to focus our analysis on what we are particularily interested in.

One of the things that we learned in the project is how long preprocessing data can take, particularily when merging multiple sources together. The time to prepare all of the data to be merged took considerable effort, not to mention the data cleaning that needed to be done on each of the data sets after merging. One trick that we used in merging the data together was to create unique variables on which to merge the datasets together. The creation of a 'merge key' may lead to slightly more work in going through the data and figuring out which fields to keep or to delete, but saves an incredible amount of time in the merging. With this knowledge in hand, we feel much more prepared to complete similar preprocessing tasks in the future. 

Another challenge that we came accross is the lack of long term data. As mentioned above, having more years of data would allow us to complete a more in depth analysis, particularily on the NFL playoffs and the super bowl. 

A final future deliverable that we would like to work on is an analysis of the number of penalties called at different differences in the score. Essentially the underlying question that we would look to be answering is if there are more penalties called in a close game or not. Unfortunately our data did not contain the score at each point in the game, so we would have to look elsewhere for the data in order to complete the analysis. Once merging this new data with our current data, we would be able to determine whether more penalties are called in a close game.
