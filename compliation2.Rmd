...
ggplot(BradData,aes(x=reorder(Team,Difference),y=Difference,color=Outlier))+geom_point(size=2.5)+coord_flip()+theme_light()+ggtitle('Brad Allen: Difference Between Away and Home by Team')+ylab('Average Penalties Away - Average Penalties Home')+xlab('Team')+geom_hline(yintercept = mean(BradData$Difference),color = 'red', alpha = 0.3)+theme_ref

```

When analyzing Figure 8, it is important to note that we are now zoomed in on the outliers of Brad Allen's referee crew, which itself is an outlier compared to all other referee crews. The outliers for Brad Allen's referee crew include Tennessee (`TEN`), Los Angeles Charges (`LAC`), and Detroit (`DET`). By analyzing points on this dot plot from left to right, we note that Brad Allen's referee crew calls more away penalties than it does home penalties. We can see that on average, Brad Allen's crew calls more penalties on away teams than home teams, but there are several outliers where they call many more penalties against the home team than the away team. This type of analysis is useful to dig into more detail about the habits of each referee crew, and further analysis for whether being the away team is a disadvantage in terms of penalties.
