...
```{r variances, anchor='Figure', fig.width = 8, fig.height = 17, fig.cap="Figure 2: Percentage of Increase/Decrease from Average Penalty Type", fig.align= 'center'}
...
It is common to think that teams are often called for more penalties when they are playing on the road as opposed to playing in their home city. This difference can be a function of player familiarity with the field, miscommunication between members of the away team due to loudness of the home crowd, and/or referee crew reaction to crowd noise encouraging or discouraging certain calls. The percentage of calls against home teams and away teams by penalty type are shown in the Figure 3 below:

```{r, anchor='Figure', fig.cap="Figure 3: Percentage of Penalties Called Against the Home/Away Teams by Penalty Type", fig.align= 'center', fig.width = 8}
...
A common question by a fan of the NFL, like any sport, is whether or not a specific referee crew is uneven against particular teams. To analyze this question, we needed to combine two categorical variables, those being the particular referee crew and the particular team that they were reffing, in order to visualize the differences in officiating. To do this we completed the following methodology:
...
Figure 6 shows box plots to show the different number of penalties each referee crew calls on average against particular teams.

```{r ref_by_team, anchor='Figure', fig.cap="Figure 6: Average Number of Penalties Given by Referee by Team", fig.align= 'center', fig.height = 7}
...
ggplot(penRef, aes(x=reorder(PenaltyTeam,Median),y=PenPerGame))+geom_boxplot(fill=fill,colour=line,outlier.colour = "red4", outlier.alpha=1,outlier.shape = 18,alpha=0.5,outlier.size = 2)+coord_flip()+ggtitle("Avg Number of Penalties Called by Referee by Team")+xlab('Team')+ylab('Average Number of Penalties Given by Referee')+theme_ref

```

In Figure 6 we observe the distributions of how teams are officiated on average. We can see that Seattle (`SEA`) has the highest median number of penalties called against them on average, while New England (`NE`) has the lowest. Remember that each one of the outliers shown above represents a particular referee crew that calls an abnormal number of penalties against a team. This will become more apparent in Figure 7 below. Note that the variance of the average number of penalties against varies greatly. For example Buffalo (`BUF`) has large variation in the ways in which referees treat them, while it appears that most referees treat Carolina (`CAR`) relatively similarily.

The outliers in Figure 6 are particularly interesting. This is because finding referee crews that act normally is not something notable, but if we are able to identify abnormal patterns in officiating then this may be of significant value. As such, in Figure 7 we narrow down the above boxplot to only those that have outliers.
...
subsetTeams %>%
  ...
  ggplot(., aes(x = factor(PenaltyTeam), y = PenPerGame)) +
    ...
    geom_text(aes(label = outlier), na.rm = TRUE,hjust = 1.1,size=3)+xlab('Team')+ylab('Average Penalty Per Game by Referee')+ggtitle('Avg Number of Penalties by Referee by Team: Outliers')+theme_ref

```

It is important to recall that this graph of boxplots is the same as in `r figr('ref_by_team', prefix=TRUE)`, only with the axes switched and the data now subsetted to only include those teams with outliers. Each of the outlier referees are labeled. One potential explanation for these deviances could be that particular referee crews are more likely to call specific penalties (for example, pass interference), while some teams are more likely to commit certain penalties. Such cases would indeed create outliers.  Another possible explanation is that there could be a limited number of data points for particular referee/team combinations, but this will be remedied as more data can be added over time.

We see that there are a few referee crews that appear on the graph multiple times. For example, John Hussey's crew shows up three times, appearing to call more penalties on the New York Giants (`NYG`) and Atlanta (`ATL`), while calling less penalties on New England (`NE`). This could mean that the crew is slightly less consistent in their penalty calling than other referee crews.

###Section 4F: Analysis by Referee by Team - Home vs Away

We were also interested in extending the analysis of referee calls by team, and thus decided to look at whether certain referees had a large difference in the number of penalties that they called on teams playing on the road versus the number of penalties that they called on teams playing at home. We used a similar methodology as described in the previous section. First we determined the average number of penalties called when teams are away and the average number of penalties called when teams are at home. We then took the difference between means (Average # Away - Average # Home). This gave us a distribution of the differences between home penalties and away penalties for each referee and each team. Because this naturally produces a large number of categorical variables, we decided to then focus only on one referee: Brad Allen. We chose Brad Allen because of the his crew had a large range between the most heavily home-favoured team and the most heavily away-favoured team. You can see the average difference between away and home penalties by team for Brad Allen in figure 8 below.

```{r ref_home_away_team, anchor='Figure', fig.cap="Figure 8: Brad Allen Average Penalties Away - Average Penalties Home", fig.align= 'center', fig.height= 7, fig.width= 6}
...
ggplot(BradData,aes(x=reorder(Team,Difference),y=Difference))+geom_point(size=2.5)+coord_flip()+ggtitle('Brad Allen: Difference Between Away and Home by Team')+ylab('Average Penalties Away - Average Penalties Home')+xlab('Team')+geom_hline(yintercept = mean(BradData$Difference),color = 'red', alpha = 0.3)+theme(axis.title.x = element_text(size = 16),axis.title.y = element_text(size = 16), plot.title = element_text(hjust = 0.5, size = 15), panel.background = element_rect(fill = "white", colour = "white", size = 0.5, linetype = "solid"),  panel.grid.major = element_line(size = 0.5, linetype = 'solid', colour = "gray90"), panel.grid.minor = element_line(size = 0.25, linetype = 'solid', colour = "gray90"))


```

When analyzing Figure 8, remember that we are now looking at the difference between home and away penalties for one individual referee: Brad Allen. We can see that the range of difference can be quite large. The expected value of the distribution above is 0, but some teams in particular (`TEN`,`OAK`,`DET`,`LAC`) have large deviations from 0. We can see that on average, Brad Allen's crew calls more penalties on away teams than home teams, but there are several outliers where they call many more penalties against the home team than the away team. This type of analysis is useful to dig into more detail about the habits of each referee crew, and further analysis for whether being the away team is a disadvantage in terms of penalties. With future analysis, we hope to learn more about how each individual crew treats the away team when compared to the home team.

###Section 4G: Analysis by Yard Line

Next we examine the distribution of penalties that are called by location on the field. Figure 9 below shows this distribution, where a value of 0 means the penalty was called as close as possible to the offensive team's own endzone, and a value of 100 means the penalty was at the goal line of the attacking endzone.
```{r yardlines_1, anchor='Figure', fig.cap="Figure 9: Penalties by Yard Line", fig.width= 8, fig.align= 'center'}
df <- refData
...
ggplot(df, aes(x=YardLine)) +
  ...
  geom_density(aes(color = "Density Curve - Yardline Data"), lwd=1.0) +
...
ggplot(df_subset, aes(x=mean_yard_line,
  ...
  theme_ref +
  geom_vline(aes(xintercept = mean(mean_yard_line), color = "Average Yardline of \nPenalty Calls Accross \nAll Referees")) +  labs(color = "Vertical Line") + scale_color_manual(values=c("black"))
```

Figure 10 shows the distribution of average yard lines at which penalties are called by each referee. The graph shows that the average yard line of penalties for all referees is between 46 and 49. This finding is consistent with the results from the histogram of `r figr('yardlines_1', prefix=TRUE)`, where we see that penalties are more frequently called at yard lines closer to the offensive team's endzone than to the defensive team's endzone.
...
A final future deliverable that we would like to conduct is an analysis of the number of penalties called at different times in NFL games based on current score. Essentially, we hope to be able to answer whether or not more penalties are called when the score of a game is close. Unfortunately our dataset did not contain the score at each point in the games, so we would have to look elsewhere for granular data in order to complete such an analysis.
