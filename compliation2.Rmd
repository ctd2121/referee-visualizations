...
As seen in the above bar chart, most penalties are called in quarters 2 and 4 of games. Quarter 4 is not too surprising, because the game is nearing conclusion at this point and we expect that teams have a higher risk tolerance as they attempt to end the game as the victor. On the other hand, the fact that quarter 2 has the highest penalty frequency is somewhat surprising because one would intuitively think that the game progresses fluidly at this point, having already played a full quarter and thus teams have gotten over any states of hesitancy or jitters. It is also surprising that quarter 1 yields the lowest penalty frequency, but we can pose a couple different hypotheses for this pattern: perhaps pace of play is legitimately slower in the first quarter than in the other three, or perhaps there is some inherent bias in referees not wanting to begin games by immediately calling penalties.

Also included in this graph is the penalty frequency for overtime periods. Although it appears that penalties are called less in overtime than they are in normal quarters, it is important to note that overtime generally does not last the full length of a regular quarter, because it concludes once there is a leading team after each team has gotten the chance to possess the ball. The game ends in a tie if there is no leader after a full period of overtime (which is equal to the length of a regular quarter), but this occurs quite infrequently. Thus, the frequency of penalties called in overtime seems proportional to that of quarters 1 through 4 when we account for duration of play.
