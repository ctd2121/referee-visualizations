...
One challenge that we faced was vizualizing data for so many categorical variables. Having 32 teams and 16 referee crews, means that visualizing anything beyond a boxplot becomes very difficult. One future path that we would like to take would be to create additional dynamic visualizations for our data. Allowing users to slice the data on different variables would allow us to show more, without overloading the report with charts. Alternatively, instead of looking at all teams and referees, in the future we could narrow out analysis to look at 
